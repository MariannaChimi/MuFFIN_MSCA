...
# # grouped boxplot
# ggplot(trainingDF, aes(x=StatesNames, y=PitchDiff, fill=ID_Ind)) +
#   geom_boxplot()+ theme(legend.position = "none")

write.csv(trainingDF, "/Users/mariannachimienti/MarieCurie/TrainingLittle2020.csv", row.names = FALSE)
...
rf_model <- 
  ...
  rand_forest(trees = 1500, mtry=4) %>% #try different values
...
save.image(file="/Users/mariannachimienti/MarieCurie/RF_Results/RF_Little2020_1500T.RData")
quit(save="no")

################### ################### ###################
################### ################### ###################
########### load workspace and test on Seasons
##################### read file
################### ################### ###################
################### ################### ###################
################### ################### ###################
...
mem_used() 
#save.image(file="/Users/mariannachimienti/MarieCurie/RF_Results/RF_Little2020_NewPitch2.RData")
#### on mac to open several R windows, open terminal and paste:open -n /Applications/R.app
...
for (TripID in 31:length(filenames)) {#length(filenames)
  accData <- fread(filenames[TripID], header = TRUE)
  rownames(accData)<-seq(1,nrow(accData),1)
...
write.csv(AccuracyDF, "/Users/mariannachimienti/MarieCurie/AccuracyDFLittle_2020_2.csv", row.names = FALSE)
write.csv(AllBudgets, "/Users/mariannachimienti/MarieCurie/AllBudgetsLittle_2020_2.csv", row.names = FALSE)
quit(save="no")


AccuracyDF_1<-read.csv("/Users/mariannachimienti/MarieCurie/AccuracyDFLittle_2020_1.csv")
AccuracyDF_2<-read.csv("/Users/mariannachimienti/MarieCurie/AccuracyDFLittle_2020_2.csv")

AllAccuracyDF<-rbind(AccuracyDF_1,AccuracyDF_1)
AllAccuracyDF<-AllAccuracyDF[which(!is.na(AllAccuracyDF$Accuracy)),]

BudgetsDF_1<-read.csv("/Users/mariannachimienti/MarieCurie/AllBudgetsLittle_2020_1.csv")
BudgetsDF_2<-read.csv("/Users/mariannachimienti/MarieCurie/AllBudgetsLittle_2020_2.csv")

BudgetsDF_All<-rbind(BudgetsDF_1,BudgetsDF_2)
head(BudgetsDF_All)
BudgetsDF_All<-BudgetsDF_All[which(!is.na(BudgetsDF_All$Still)),]

write.csv(AllAccuracyDF, "/Users/mariannachimienti/MarieCurie/RF_Results/AccuracyDFLittle_2020_All.csv", row.names = FALSE)
write.csv(BudgetsDF_All, "/Users/mariannachimienti/MarieCurie/RF_Results/AllBudgetsLittle_2020_All.csv", row.names = FALSE)
...
write.csv(AccuracyDF, "/Users/mariannachimienti/MarieCurie/AccuracyDFLittle_2019_All.csv", row.names = FALSE)
write.csv(AllBudgets, "/Users/mariannachimienti/MarieCurie/AllBudgetsLittle_2019_All.csv", row.names = FALSE)
...
############## ############## ############## ############## ############## 
Accuracy_2020DF <-fread("/Users/mariannachimienti/MarieCurie/RF_Results/AccuracyDFLittle_2020_All.csv",header=TRUE)
Accuracy_2020DF$Season<-1

Accuracy_2019DF <-fread("/Users/mariannachimienti/MarieCurie/RF_Results/AccuracyDFLittle_2019_All.csv",header=TRUE)
Accuracy_2019DF$Season<-2

AccuracyAll<-rbind(Accuracy_2020DF,Accuracy_2019DF)
...
Budget_2020DF <-fread("/Users/mariannachimienti/MarieCurie/RF_Results/AllBudgetsLittle_2020_All.csv",header=TRUE)
Budget_2020DF$Season<-1

Budget_2019DF <-fread("/Users/mariannachimienti/MarieCurie/RF_Results/AllBudgetsLittle_2019_All.csv",header=TRUE)
Budget_2019DF$Season<-2

BudgetAll<-rbind(Budget_2020DF,Budget_2019DF)
BudgetAll$Season<-as.factor(BudgetAll$Season)
unique(BudgetAll$beh)
table(BudgetAll$beh)
BudgetAll$beh<-as.factor(BudgetAll$beh)

BudgetAll<-BudgetAll[-which(BudgetAll$beh=="Preen/highFlap_L")]
BudgetAll<-BudgetAll[-which(BudgetAll$beh=="Walk")]
BudgetAll<-BudgetAll[-which(BudgetAll$beh=="Stand")]
BudgetAll<-BudgetAll[-which(BudgetAll$beh=="Preen/highFlap_L")]

ggplot() + 
  geom_boxplot(aes(y = bud_value, x = beh, fill = Method), data = BudgetAll)+
  ylab("proportion")+xlab("")+theme(text = element_text(size=15))
