...
#### on mac to open several R windows, open terminal and paste:open -n /Applications/R.app
 ...
 for (i in 33:length(filenames)) { #change this line if you want more or less files running
   ...
   #  names(accData) <- gsub(x = names(accData), pattern = "\\?", replacement = "_") 
    ...
    #### #### #### #### #### #### #### #### #### #### 
    LandPar<-1 ##### Little penguin do not go on ice blocks
    ...
    #   
    #   if((max(subDataUWList[[nList]]$Pitch)>=60 & meanT>0) | (mean(subDataUWList[[nList]]$Pitch)>=20 & timeChunk> 750)) {
    #     
    ...
    #       
    #       if( any(DataChunk_NA_List[[nListNA]]$Pitch<thr_pitch_lying)){
    ...
    pathRes <-
      paste("/Users/mariannachimienti/MarieCurie/EM_Results_Little_2019/",
            ID_Ind,
            sep = "")
