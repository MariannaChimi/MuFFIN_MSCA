...
##############################################################################################################
#### read file
accData<-fread("/Volumes/Mari_MC2/DataAdelie_25Hz_2018-2019/pa19_009y_F55_13_25Hz.csv",header=TRUE)
head(accData)
tail(accData)
str(accData)
...
#dev.new()
ggplot(accData[2000000:2300000,]) +geom_line(aes(x=DatesPos, y=depth25Hz), size=1)#beginning
ggplot(accData[250000:500000,]) +geom_point(aes(x=DatesPos, y=Temp), size=1)#beginning
ggplot(accData[2200000:2300000,]) +geom_line(aes(x=DatesPos, y=Pitch), size=1)#beginning

ggplot(accData[4000000:4500000,]) +geom_line(aes(x=DatesPos, y=depth25Hz), size=1)#middle
...
accData_an<-accData[which(accData$DatesPos>=as.POSIXct("2019-01-10 02:30:00.00", format='%Y-%m-%d %H:%M:%OS',tz="UTC")),]
accData_an<-accData_an[which(accData_an$DatesPos<as.POSIXct("2019-01-11 02:25:00.00", format='%Y-%m-%d %H:%M:%OS',tz="UTC")),]
...
write.csv(accData_an,"/Volumes/Mari_MC2/DataAdelie_Trips_2018-2019/pa19_009y_F55_13_25HzTrip1.csv", row.names = TRUE)
